...
from ..bolt import GPath, DataDict, cstrip, deprint, sio, Path, decode, struct_pack, struct_unpack
...
class ModInfos(FileInfos):
    ...
    try:
        file_pattern = re.compile(ur'\.(' + '|'.join(x[1:] for x in bush.game.espm_extensions) +
                                  ur')(.ghost)?$', re.I | re.U)
    except AttributeError:
        pass
    ...
    def lo_activate(self, fileName, doSave=True, _modSet=None, _children=None,
        ...
        try:
            ...
            if len(acti_filtered_espm) == _max_espms:
                raise PluginsFullError(u'%s: Trying to activate more than 255 espms' % fileName)
    ...
    def lo_activate_exact(self, modNames):
        ...
        extra = acti_filtered_espm[_max_espms:]
        #--Save
        if extra:
            listToSelect = [x for x in listToSelect if x not in set(extra)]
